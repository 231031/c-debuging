...
// lists is information of book that read from text file
...
int returnBook(LIST_LIBARY *lists)
{
    ...
    fclose(fpStatus);
    free(lists);
    // list->activity.borrower = reTurn;
    // list->status = "Available";
}

/*
LIST_LIBARY* listCreate(int initialCapacity);
void listAppend(LIST_LIBARY* list, int value);
void listSeatitleSearchAt(LIST_LIBARY* list, int index, int value);
void listRemoveAt(LIST_LIBARY* list, int index);
void listEdit(LIST_LIBARY* list, int index);
int listGetAt(LIST_LIBARY* list, int index);
int listSize(LIST_LIBARY* list);
void listDestroy(LIST_LIBARY* list);


LIST_LIBARY* listCreate()
{
    LIST_LIBARY* list = (LIST_LIBARY*) calloc(10, sizeof(LIST_LIBARY));
    list->name = (char *) calloc(MAX, sizeof(char));
    list->writer = (char *) calloc(MAX, sizeof(char));
    list->year = (char *) calloc(10, sizeof(char));
    if (list == NULL)
    {
        printf("list cannot allocate\n");
        return 0;
    }
    return list;
}*/
/*
void bookAppend(LIST_T* list, int value);
void bookInsert(LIST_T* list, int index, int value);
void bookRemove(LIST_T* list, int index);
void bookSearch(LIST_T* list, int index);
void bookEdit(LIST_T* list);
void bookReturn(LIST_T* list);
void bookBorrow(LIST_T* list);
*/
